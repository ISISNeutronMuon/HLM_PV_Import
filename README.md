...
**Notes**: 
* Do not forget to set up the database as well, the schema can be found in `\\isis\inst$\Kits$\CompGroup\Helium Level Meters\For Review`. When first starting the manager, set-up the DB Connection settings. The DB user & password are saved in the windows registry as parameters of the service.  
